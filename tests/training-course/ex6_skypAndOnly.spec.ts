...
/*
test.skip -> Saltar la ejecución de un test
test.only -> Ejecutar solo ese test de la batería de pruebas
grep -> Ejecutar por tag en un fichero de tests. Se deberá poner un @ dentro de las '' del nombre del test
     -> Ejecutar por consola con npx playwright test --grep @refactor
test.skip -> Para saltar por ejemplo la ejecución de un navegador concreto. Incluimos browserName en el async
     -> (browserName === 'chromium', 'No está todavía implementado para Chrome')
test.info().annotations.push({
  type: 'User Story 131234',
  description: 'El usuario puede seleccionar un día del dropdown Días de Semana'
});
await test.info().attach('screenshot, {}) -> Añadimos una captura a la ejecución
*/
...
(async () => {
    ...
    test.describe('Aserciones/Validaciones en el Automation Sandbox', () => {
        //test.skip(({ browserName }) => browserName === 'chromium', 'No está todavía implementado para Chrome');
        ...
        test('Aserciones con el botón ID dinámico @refactor', async ({ page, browserName }) => {
            //test.skip(browserName === 'chromium', 'No está todavía implementado para Chrome');
            await test.step('Dado que navego al Sandbox de Automation Free Range Tester', async () => {
                await page.goto('https://thefreerangetester.github.io/sandbox-automation-testing/');
            })
